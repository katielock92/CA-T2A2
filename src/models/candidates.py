...
class CandidateSchema(ma.Schema):
    ...
    phone_number = fields.String(validate=And(Length(min=10, error="Phone number must be between 10-20 characters in length"),Length(max=20, error="Phone number must be between 10-20 characters in length"),Regexp("^[0-9() -+]+",error="Phone number can only contains numbers and certain special characters - please try again.",),),)
